...
function CategoriasCdaInactivos() {
    ...
    const confirmActivar = () => {
        ...
        axios.put(`${apiUrl}/categoria_cda/estado/${selectedId}`, { estado: "Activo" }, {
            headers: {
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/categoria_cdas`, {
            ...
            .then(response => {
                setCategoriasCda(response.data);
                setLoading(false);
            })
            .catch(error => {
    ...
    const inactiveCategoriasCda = categoriasCda.filter(categoriaCda => categoriaCda.estado === 'Inactivo');
    const inactiveCategoriasCdaCount = inactiveCategoriasCda.length;
    ...
    const renderCategorias = () => (
        <div className="container mx-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Categorías CDA Inactivos</h5>
            <br />
            <DataTable 
                value={inactiveCategoriasCda}
                paginator
                rows={5}
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Categorías CDA Inactivos: {inactiveCategoriasCdaCount}</Button>
                <Button href='/categorias-cda'>
                    <HiOutlineArrowNarrowLeft className="mr-2 h-5 w-5" />Ver Categorías CDA Activos
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmActivar} msg={'¿Desea activar esta categoría CDA?'} />
            </section>
        </Layout>
    );
}

export default CategoriasCdaInactivos;