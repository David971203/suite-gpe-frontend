...
function CategoriasCdaActivos() {
    const [categoriasCda, setCategoriasCda] = useState([]);
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`${apiUrl}/categoria_cda`, {
        ...
        .then(response => {
            setCategoriasCda([ response.data.categoria_cda,...categoriasCda]);
    ...
    const fetchUserData = async (selectedId) => {
        ...
        await axios.get(`${apiUrl}/categoria_cda/${selectedId}`, {
            headers: {
    ...
    const handleEdtSubmit = (e) => {
        ...
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/categoria_cda/${selectedId}`, {
            nombre: formData.nombre,
            estado: formData.estado,
        }, {
            headers: {
        ...
        .then(response => {
            setCategoriasCda((prevcategoriasCda) => 
                prevcategoriasCda.map(categoriaCda => categoriaCda.id === selectedId ? response.data.categoria_cda : categoriaCda)
            );
            setToastMessage(response.data.message);
            setShowToastSUCC(true);
            setTimeout(() => setShowToastSUCC(false), 5000);
    ...
    const confirmDelete = () => {
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/categoria_cda/estado/${selectedId}`, { estado: "Inactivo" }, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
            .then(response => {
                setCategoriasCda(categoriasCda.filter(categoriaCda => categoriaCda.id !== selectedId));
                setOpenModal(false);
                setToastMessage(response.data.message);
                setShowToastERR(true);
                setTimeout(() => setShowToastERR(false), 5000);
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/categoria_cdas`, {
            ...
            .then(response => {
                const sortedcategoriasCda = response.data.sort((a, b) => b.id - a.id);
                setCategoriasCda(sortedcategoriasCda);
                setLoading(false);
            })
            .catch(error => {
    ...
    const activeCategoriasCda = categoriasCda.filter(categoriaCda => categoriaCda.estado === 'Activo');
    const activeCategoriasCda_count = activeCategoriasCda.length;
    ...
    const renderCategoria = () => (
        <div className="container mx-auto h-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Categorías CDA</h5>
            <br />
            <Button onClick={() => setOpenNewModal(true)} className='mb-2'>
                Nueva Categoría CDA <HiOutlinePlus className="ml-2 h-5 w-5" />
            </Button>
            <DataTable 
                value={activeCategoriasCda}
                paginator
                rows={5}
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Categorías CDA Activas: {activeCategoriasCda_count}</Button>
                <Button href='/categorias-cda/inactive'>
                    Ver Categorías CDA Inactivas <HiOutlineArrowNarrowRight className="ml-2 h-5 w-5" />
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmDelete} msg={'¿Desea eliminar esta categoría CDA?'} />
            </section>
        </Layout>
    );
}

export default CategoriasCdaActivos;