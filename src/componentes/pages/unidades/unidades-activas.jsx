...
function UnidadesActivas() {
    ...
    const [isPrincipalEdt, setIsPrincipalEdt] = useState(false);
    ...
    const handleSubmit = (e) => {
        ...
        if(formData.principal === 0 && parseInt(formData.unidad_padre_id) === 0){
            setcolorInputText('failure');
                setmsgInputText('Debe escojer una unidad padre si la unidad no es principal');
        ...
        const token = localStorage.getItem('token');
        axios.post(`${apiUrl}/unidad`, {
            nombre: formData.nombre,
            codigo_sentai: formData.codigo_sentai,
            estado: formData.estado,
            principal:formData.principal,
            unidad_padre_id: parseInt(formData.unidad_padre_id),
        ...
        .then(response => {
            setUnidades([ response.data.unidad,...unidades]);
    ...
    const fetchUserData = async (selectedId) => {
        ...
        const unidad_padre_tmp = 0;
        await axios.get(`${apiUrl}/unidad/${selectedId}`, {
            headers: {
            ...
            .then(response => {
                ...
                setFormData({
                    ...
                    codigo_sentai: response.data.codigo_sentai,
                    principal: response.data.principal,
                    estado: response.data.estado,
                    unidad_padre_id: response.data.unidad_padre_id,
                    sector_id: response.data.sector_id
                });

                

                setIsPrincipalEdt(response.data.principal === 1);
    ...
    const handleEdtSubmit = (e) => {
        ...
        if(parseInt(formData.principal) === 0 && parseInt(formData.unidad_padre_id) === 0){
            setcolorInputText('failure');
                setmsgInputText('Debe escojer una unidad padre si la unidad no es principal');
        ...
        const token = localStorage.getItem('token');
        let uni_p = 0;
        if(formData.unidad_padre_id === null)
            uni_p = 0;
        else
            uni_p = parseInt(formData.unidad_padre_id);

        axios.put(`${apiUrl}/unidad/${selectedId}`, {
            nombre: formData.nombre,
            codigo_sentai: formData.codigo_sentai,
            estado: formData.estado,
            principal:formData.principal,
            unidad_padre_id: uni_p,
            sector_id: formData.sector_id
        }, {
            headers: {
                'Authorization': `Bearer ${token}`
        ...
        .then(response => {
            ...
            setUnidades((prevUnidades) => 
                prevUnidades.map(unidad => unidad.id === selectedId ? response.data.unidad : unidad)
            );
            setToastMessage(response.data.message);
            setShowToastSUCC(true);
            setTimeout(() => setShowToastSUCC(false), 5000);
    ...
    const confirmDelete = () => {
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/unidad/estado/${selectedId}`, { estado: "Inactivo" }, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
            .then(response => {
                setUnidades(unidades.filter(unidad => unidad.id !== selectedId));
                setOpenModal(false);
                setToastMessage(response.data.message);
                setShowToastERR(true);
                setTimeout(() => setShowToastERR(false), 5000);
              ...
              {unidad.unidades && unidad.unidades.length > 0 ? (
                ...
                {unidad.unidades && unidad.unidades.length > 0 && (
                            <Accordion alwaysOpen>
                                {renderAccordion(unidad.unidades)}
                            </Accordion>
                        )}
              </Accordion.Content>
            </Accordion.Panel>
    ...
    const handleCheckboxChangeEdt = (e) => {
        setIsPrincipalEdt(e.target.checked);
        if (e.target.checked) {
            // Restablecer el valor del select de unidad padre
            formData.principal = 1;
            handleInputChange({ target: { name: 'unidad_padre_id', value: '0' } });
    ...
    return (
        <Layout>
            ...
            <section className=" bg-white dark:bg-gray-800 relative shadow-md rounded-lg overflow-y-auto">
                ...
                <Modal show={openNewModal}  size='md'onClose={() => {setOpenNewModal(false); resetFormData();}}>
                    ...
                    <Modal.Body>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <Label htmlFor="codigo_sentai">Código Sentai</Label>
                                <TextInput id="codigo_sentai" name="codigo_sentai" value={formData.codigo_sentai}  
                                
                                onChange={handleInputChange}  onInput={(e) => e.target.setCustomValidity('')} aria-describedby="helper-text-explanation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"/>
                            </div>
                            <div className="flex items-center gap-2">
                                <Label htmlFor="accept" className="flex">
                                    Principal
                                </Label>
                                <Checkbox id="accept" checked={isPrincipal} value={formData.principal} onChange={handleCheckboxChange} />
                            </div>
                            <div>
                                <Label htmlFor="unidad_padre_id">Unidad Padre</Label>
                                <Select id="unidad_padre_id" name="unidad_padre_id" value={formData.unidad_padre_id} color={colorInputText} onChange={handleInputChange} disabled={isPrincipal} onInput={(e) => e.target.setCustomValidity('')}>
                                    <option value="0">-Seleccione una unidad-</option>
                                    {activeUnidadesSelect.map(unidad => (
                                        <option key={unidad.id} value={unidad.id}>{unidad.nombre}</option>
                                    ))}
                                </Select>
                                <p className="red-text">{msgInputText}</p>
                            </div>
                            <div>
                    ...
                    <Modal.Body>
                        <form onSubmit={handleEdtSubmit} className="space-y-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Checkbox id="accept" checked={isPrincipalEdt} value={formData.principal} onChange={handleCheckboxChangeEdt} />
                            </div>
                            <div>
                                <Label htmlFor="unidad_padre_id">Unidad Padre</Label>
                                <Select id="unidad_padre_id" name="unidad_padre_id" value={formData.unidad_padre_id}  color={colorInputText} onChange={handleInputChange} disabled={isPrincipalEdt} onInput={(e) => e.target.setCustomValidity('')}>
                                    <option value={0}>-Seleccione una unidad-</option>
                                    {activeUnidadesSelect.map(unidad => (
                                        formData.nombre !== unidad.nombre ?
                                        <option key={unidad.id} value={unidad.id}>{unidad.nombre}</option>
                                        : null
                                    ))}
                                </Select>
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmDelete} msg={'¿Desea eliminar esta unidad, se eliminaran también todos los usuarios asociados a esta?'} />
