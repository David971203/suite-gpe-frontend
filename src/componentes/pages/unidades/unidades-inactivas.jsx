...
function UnidadesInactivas() {
    ...
    const confirmActivar = () => {
        ...
        axios.put(`${apiUrl}/unidad/estado/${selectedId}`, { estado: "Activo" }, {
            headers: {
    ...
    const inactiveUnidades = unidades.filter(unidad => unidad.estado === 'Inactivo');
    const inactiveUnidadesCount = inactiveUnidades.length;
    ...
    const renderUnidades = () => (
        <div className="container mx-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Unidades Inactivas</h5>
            <br />
            <DataTable 
                value={inactiveUnidades}
                paginator
                rows={5}
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Unidades Inactivas: {inactiveUnidadesCount}</Button>
                <Button href='/unidades'>
                    <HiOutlineArrowNarrowLeft className="mr-2 h-5 w-5" />Ver Unidades Activas
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmActivar} msg={'¿Desea activar esta unidad?'} />
            </section>
        </Layout>
    );
}

export default UnidadesInactivas;