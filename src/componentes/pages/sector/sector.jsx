...
function Sectores() {
    ...
    const handleSubmit = (e) => {
        ...
        .then(response => {
            setSectores([ response.data.sector,...sectores]);
    ...
    const fetchUserData = async (selectedId) => {
        ...
        await axios.get(`${apiUrl}/sector/${selectedId}`, {
            headers: {
    ...
    const handleEdtSubmit = (e) => {
        ...
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/sector/${selectedId}`, {
            nombre: formData.nombre,
        }, {
            headers: {
                'Authorization': `Bearer ${token}`
        ...
        .then(response => {
            setSectores((prevSector) => 
                prevSector.map(sector => sector.id === selectedId ? response.data.sector : sector)
            );
            setToastMessage(response.data.message);
            setShowToastSUCC(true);
            setTimeout(() => setShowToastSUCC(false), 5000);
    ...
    const confirmDelete = () => {
        const token = localStorage.getItem('token');
        axios.delete(`${apiUrl}/sector/${selectedId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
            .then(response => {
                setSectores(sectores.filter(sector => sector.id !== selectedId));
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/sectors`, {
            ...
            .then(response => {
                const sortedsector = response.data.sort((a, b) => b.id - a.id);
                
                setSectores(sortedsector);
    ...
    const sectorCount = sectores.length;
    ...
    const renderSectores = () => (
        <div className="container mx-auto h-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Sectores</h5>
            <br />
            <Button onClick={() => setOpenNewModal(true)} className='mb-2'>
                Nuevo Sector <HiOutlinePlus className="ml-2 h-5 w-5" />
            </Button>
            <DataTable 
                value={sectores}
                paginator
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Sectores: {sectorCount}</Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmDelete} msg={'¿Desea eliminar este sector?'} />
            </section>
        </Layout>
    );
}
