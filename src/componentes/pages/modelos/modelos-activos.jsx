...
function ModelosActivos() {
    ...
    const handleSubmit = (e) => {
        ...
        .then(response => {
            setModelos([ response.data.modelo,...modelos]);
    ...
    const fetchUserData = async (selectedId) => {
        ...
        await axios.get(`${apiUrl}/modelo/${selectedId}`, {
            headers: {
    ...
    const handleEdtSubmit = (e) => {
        ...
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/modelo/${selectedId}`, {
            nombre: formData.nombre,
            estado: formData.estado,
            marca_id: formData.marca_id
        }, {
        ...
        .then(response => {
            setModelos((prevModelos) => 
                prevModelos.map(modelo => modelo.id === selectedId ? response.data.modelo : modelo)
            );
            setToastMessage(response.data.message);
            setShowToastSUCC(true);
            setTimeout(() => setShowToastSUCC(false), 5000);
    ...
    const confirmDelete = () => {
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/modelo/estado/${selectedId}`, { estado: "Inactivo" }, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
            .then(response => {
                setModelos(modelos.filter(modelo => modelo.id !== selectedId));
                setOpenModal(false);
                setToastMessage(response.data.message);
                setShowToastERR(true);
                setTimeout(() => setShowToastERR(false), 5000);
    ...
    useEffect(() => {
        const token = localStorage.getItem('token');
        axios.get(`${apiUrl}/modelos`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
            .then(response => {
                const sortedModelos = response.data.sort((a, b) => b.id - a.id);
    ...
    const activeModelos = modelos.filter(modelo => modelo.estado === 'Activo');
    const activeModelos_Count = activeModelos.length;
    ...
    const renderModelos = () => (
        <div className="container mx-auto h-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Modelos</h5>
            <br />
            <Button onClick={() => setOpenNewModal(true)} className='mb-2'>
                Nuevo Modelo<HiOutlinePlus className="ml-2 h-5 w-5" />
            </Button>
            <DataTable 
                value={activeModelos}
                paginator
                ...
                filters={filters}
                globalFilterFields={['nombre','marca.nombre']}
                header={renderHeader()}
                filterDisplay="row"
            ...
            >
                ...
                <Column 
                    field="marca.nombre" 
                    header="Marca" 
                    body={(rowData) => renderWithTooltip(rowData, 'marca.nombre')}
                    className="p-col text-sm font-medium text-gray-900 px-6 py-4"
                />
                <Column 
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Modelos Activos: {activeModelos_Count}</Button>
                <Button href='/modelos/inactive'>
                    Ver Modelos Inactivos <HiOutlineArrowNarrowRight className="ml-2 h-5 w-5" />
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                    ...
                    <Modal.Body>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <Select id="marca_id" name="marca_id" value={formData.marca_id} onChange={handleInputChange} required onInvalid={(e) => e.target.setCustomValidity('Por favor, seleccione un elemento.')} onInput={(e) => e.target.setCustomValidity('')}>
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmDelete} msg={'¿Desea eliminar este modelo?'} />
                
            </section>
        </Layout>
    );
}
export default ModelosActivos;