...
import { HiOutlineSearch, HiOutlineArrowNarrowLeft} from "react-icons/hi";
...
function  ModelosInactivos() {
    ...
    const confirmActivar = () => {
        ...
        axios.put(`${apiUrl}/modelo/estado/${selectedId}`, { estado: "Activo" }, {
            headers: {
    ...
    const inactiveModelos = modelos.filter(modelo => modelo.estado === 'Inactivo');
    const inactiveModelos_Count = inactiveModelos.length;
    ...
    const renderModelos = () => (
        <div className="container mx-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Modelos Inactivos</h5>
            <br />
            <DataTable 
                value={inactiveModelos}
                paginator
                rows={5}
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Modelos Inactivos: {inactiveModelos_Count}</Button>
                <Button href='/modelos'>
                    <HiOutlineArrowNarrowLeft className="mr-2 h-5 w-5" />Ver Modelos Activos
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmActivar} msg={'¿Desea activar este modelo?'} />
            </section>
        </Layout>
    );
}

export default  ModelosInactivos;