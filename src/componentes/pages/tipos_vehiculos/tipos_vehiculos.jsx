...
function TiposVehiculos() {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`${apiUrl}/tipo_vehiculo`, {
        ...
        .then(response => {
            setTiposVehiculos([ response.data.tipo_vehiculo,...tiposVehiculos]);
    ...
    const fetchUserData = async (selectedId) => {
        ...
        await axios.get(`${apiUrl}/tipo_vehiculo/${selectedId}`, {
            headers: {
    ...
    const handleEdtSubmit = (e) => {
        ...
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/tipo_vehiculo/${selectedId}`, {
            nombre: formData.nombre,
        }, {
            headers: {
                'Authorization': `Bearer ${token}`
        ...
        .then(response => {
            setTiposVehiculos((prevTiposVehiculo) => 
                prevTiposVehiculo.map(tipo_vehiculo => tipo_vehiculo.id === selectedId ? response.data.tipo_vehiculo : tipo_vehiculo)
            );
            setToastMessage(response.data.message);
            setShowToastSUCC(true);
            setTimeout(() => setShowToastSUCC(false), 5000);
    ...
    const confirmDelete = () => {
        const token = localStorage.getItem('token');
        axios.delete(`${apiUrl}/tipo_vehiculo/${selectedId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
            .then(response => {
                setTiposVehiculos(tiposVehiculos.filter(tipo_vehiculo => tipo_vehiculo.id !== selectedId));
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/tipo_vehiculos`, {
            ...
            .then(response => {
                const sortedtiposVehiculo = response.data.sort((a, b) => b.id - a.id);
                
                setTiposVehiculos(sortedtiposVehiculo);
    ...
    const tipoVehiculoCount = tiposVehiculos.length;
    ...
    const renderTiposVehiculos = () => (
        <div className="container mx-auto h-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Tipos de Vehículos</h5>
            <br />
            <Button onClick={() => setOpenNewModal(true)} className='mb-2'>
                Nuevo Tipo de Vehículo <HiOutlinePlus className="ml-2 h-5 w-5" />
            </Button>
            <DataTable 
                value={tiposVehiculos}
                paginator
                rows={5}
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Tipos de Vehículos: {tipoVehiculoCount}</Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmDelete} msg={'¿Desea eliminar este tipo de vehículo?'} />
            </section>
        </Layout>
    );
}
