...
function  ActividadesCdaInactivos() {
    ...
    const confirmActivar = () => {
        ...
        axios.put(`${apiUrl}/actividad_cda/estado/${selectedId}`, { estado: "Activo" }, {
            headers: {
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/actividad_cdas`, {
            ...
            .then(response => {
                setActividadesCda(response.data);
                setLoading(false);
            })
            .catch(error => {
    ...
    const inactiveActividadesCda = actividadesCda.filter(actividadCda => actividadCda.estado === 'Inactivo');
    const inactiveActividadesCount = inactiveActividadesCda.length;
    ...
    const renderActividadesCda = () => (
        <div className="container mx-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Actividades CDA Inactivas</h5>
            <br />
            <DataTable 
                value={inactiveActividadesCda}
                paginator
                rows={5}
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Actividades CDA Inactivas: {inactiveActividadesCount}</Button>
                <Button href='/actividades-cda'>
                    <HiOutlineArrowNarrowLeft className="mr-2 h-5 w-5" />Ver Actividades CDA Activas
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmActivar} msg={'¿Desea activar esta actividad CDA?'} />
            </section>
        </Layout>
    );
}

export default  ActividadesCdaInactivos;