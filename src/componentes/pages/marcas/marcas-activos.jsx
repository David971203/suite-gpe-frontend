...
function MarcasActivos() {
    ...
    const handleSubmit = (e) => {
        ...
        .then(response => {
            setMarcas([ response.data.marca,...marcas]);
    ...
    const fetchUserData = async (selectedId) => {
        ...
        await axios.get(`${apiUrl}/marca/${selectedId}`, {
            headers: {
    ...
    const handleEdtSubmit = (e) => {
        ...
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/marca/${selectedId}`, {
            nombre: formData.nombre,
            estado: formData.estado,
        }, {
            headers: {
        ...
        .then(response => {
            setMarcas((prevMarcas) => 
                prevMarcas.map(Marca => Marca.id === selectedId ? response.data.marca : Marca)
            );
            setToastMessage(response.data.message);
            setShowToastSUCC(true);
            setTimeout(() => setShowToastSUCC(false), 5000);
    ...
    const confirmDelete = () => {
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/marca/estado/${selectedId}`, { estado: "Inactivo" }, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/marcas`, {
            ...
            .then(response => {
                const sortedmarca = response.data.sort((a, b) => b.id - a.id);
                setMarcas(sortedmarca);
                setLoading(false);
            })
            .catch(error => {
    ...
    const activemarca = marcas.filter(marca => marca.estado === 'Activo');
    const activemarca_count = activemarca.length;
    ...
    const renderMarca = () => (
        <div className="container mx-auto h-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Marcas</h5>
            <br />
            <Button onClick={() => setOpenNewModal(true)} className='mb-2'>
                Nueva Marca <HiOutlinePlus className="ml-2 h-5 w-5" />
            </Button>
            <DataTable 
                value={activemarca}
                paginator
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Marcas Activas: {activemarca_count}</Button>
                <Button href='/marcas/inactive'>
                    Ver Marcas Inactivas <HiOutlineArrowNarrowRight className="ml-2 h-5 w-5" />
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <ConfirmacionModal show={openModal} onClose={() => setOpenModal(false)} onConfirm={confirmDelete} msg={'¿Desea eliminar esta marca?'} />
            </section>
        </Layout>
    );
}

export default MarcasActivos;