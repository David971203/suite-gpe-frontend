...
function AsociarActividadesCda() {
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/actividad_cdas`, {
            ...
            .then(response => {
                ...
                setTodasActividades(sortedActividadesCda);
                setLoading(false);
            })
            .catch(error => {
    ...
    const todasactiveActividadesCda = todasActividades.filter(actividadCda => actividadCda.estado === 'Activo');
    const options = todasactiveActividadesCda.map(actividad => ({
        value: actividad.id,
    ...
    const handleSubmit = (e) => {
        ...
        if(formData.actividad == ''){
                ...
                setToastMessage('Seleccione una Actividad válida ');
                setShowToastERR(true);
                setTimeout(() => setShowToastERR(false), 5000);
            ...
            const token = localStorage.getItem('token');
            axios.put(`${apiUrl}/unidades_actividades_add/${unidad_id}/${formData.actividad}`,{}, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
    ...
    const handleEdtSubmit = (e) => {
        ...
        if(selectedId == formData.actividad){
            
            setToastMessage('La Actividad ha sido asociada a la unidad');
                setShowToastSUCC(true);
                setTimeout(() => setShowToastSUCC(false), 5000);
            ...
            axios.put(`${apiUrl}/unidades_actividades_edt/${unidad_id}/${selectedId}/${formData.actividad}`,{}, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
    ...
    const confirmDelete = () => {
        const token = localStorage.getItem('token');
        axios.put(`${apiUrl}/unidades_actividades_del/${unidad_id}/${selectedId}`,{}, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
    ...
    const renderActividadesCda = () => (
        <div className="container mx-auto h-auto px-4">
            <h5 className="text-2xl font-bold text-cyan-700 dark:text-white">Actividades CDA de la Unidad: {unidad_name}</h5>
            <br />
            ...
            <div className="flex flex-wrap gap-2">
                <Button>Actividades de la unidad: {activeActividadesCdaCount}</Button>
            </div>
        </div>
    );
    ...
    return (
        <Layout>
            <section className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg mx-auto h-auto">
                ...
                <Modal show={openNewModal}  size='lg'onClose={() => {setOpenNewModal(false); resetFormData();}}>
                    ...
                    <Modal.Body>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <Label htmlFor="nombre">Actividad CDA</Label>
                                <Select2 
                                        className="basic-single "
                                        classNamePrefix="select"
                                        defaultValue={todasactiveActividadesCda[0]}
                                        onChange={(selectedOption) => setFormData({ ...formData, actividad: selectedOption.value })}
                        ...
                        <form onSubmit={handleEdtSubmit} className="space-y-4">
                            <div>
                                ...
                                <Select2 
                                        className="basic-single "
                                        classNamePrefix="select"
                                        value={options.find(option => option.value === formData.actividad)}
                                        onChange={(selectedOption) => setFormData({ ...formData, actividad: selectedOption.value })}
...
export default AsociarActividadesCda;